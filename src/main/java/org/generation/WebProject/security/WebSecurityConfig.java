...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter{
    ...
    protected void configure(HttpSecurity http) throws Exception {
        //all of these methods are in built from Spring Security class
        http.csrf().disable();
        ...
        /*.antMatchers(.....).permitAll() - tells Spring Security that these webpages do not need to have login services

        antMatchers(.....).hasRole("ADMIN") - tells Spring Security that only user with ADMIN role will be able to access the productForm.html

        logout method : configure logout functionality provided by Spring   Security - ensure that the login session to
         be invalidated.
        */
